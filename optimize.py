# ============================================================
# VN TRADER BOT V5 — optimize.py  (v4 — WFO fixed)
#
# FIXES v4:
#   1. ProcessPoolExecutor → ThreadPoolExecutor
#      Railway free tier dùng container không hỗ trợ fork() tốt
#      → ProcessPoolExecutor bị deadlock khi chạy trong asyncio.to_thread()
#      → Đổi sang ThreadPoolExecutor: thread-safe, không fork, không deadlock
#      → backtest.py chỉ dùng numpy/pandas — GIL không phải vấn đề ở đây
#      (CPU-bound nhẹ, bottleneck thực sự là pandas overhead, không phải GIL)
#
#   2. MAX_WORKERS: 2 → 4
#      ThreadPoolExecutor không bị giới hạn vCPU như Process
#      4 threads cho Railway free tier (1 vCPU) vẫn OK vì pandas release GIL
#
#   3. Timeout guard cho từng fold trong WFO
#      Nếu 1 fold bị treo (worker hung), bot không bị tê liệt mãi mãi
#      → concurrent.futures.wait() với timeout=120s mỗi fold
#      → Fold bị timeout sẽ bị skip thay vì block toàn bộ
#
# ============================================================
...
# FIX #2: tăng lên 4 vì dùng Thread (không bị giới hạn fork)
MAX_WORKERS = 4

# FIX #3: timeout per fold (giây) — nếu 1 fold không xong sau 120s → skip
FOLD_TIMEOUT_SEC = 120
...
# ── Grid thread worker ────────────────────────────────────────
# FIX #1: Không cần top-level pickle-able nữa vì dùng ThreadPoolExecutor
# Thread worker có thể là closure hoặc lambda — đơn giản hơn nhiều
...
def _run_single_combo(
    ...
    """Chạy 1 combo (sl, tp, hold) — dùng trực tiếp trong ThreadPoolExecutor."""
    try:
        ...
        if r.total_trades < min_trades:
            return None
        return (sl, tp, hold, r.profit_factor, r.win_rate, r.total_trades)
    except Exception as e:
        logger.warning(f"[{symbol}] Combo ({sl},{tp},{hold}) error: {e}")
        return None
...
def _run_grid_parallel(
    ...
    min_trades: int,
    max_workers: int,
    timeout_sec: float = FOLD_TIMEOUT_SEC,
) -> list:
    """
    Chạy 64-combo grid search song song bằng ThreadPoolExecutor.
    Trả về list sorted by PF desc.

    FIX #1: ThreadPoolExecutor thay cho ProcessPoolExecutor
      - Không fork() → không deadlock với asyncio event loop
      - Railway container không bị treo
      - pandas/numpy release GIL → vẫn chạy song song thực sự

    FIX #3: timeout_sec — nếu toàn bộ grid không xong sau N giây → trả về kết quả hiện có
    """
    ...
    try:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(
                    _run_single_combo,
                    symbol, df, cfg, bt_cfg, df_vni,
                    rsi_mode, min_trades, sl, tp, hold,
                ): (sl, tp, hold)
                for sl, tp, hold in all_combos
            }

            # FIX #3: dùng as_completed với timeout thay vì chờ mãi
            done, pending = wait(futures, timeout=timeout_sec)

            if pending:
                logger.warning(
                    f"[{symbol}] Grid timeout ({timeout_sec}s): "
                    f"{len(done)}/{len(futures)} combos done, "
                    f"{len(pending)} cancelled"
                )
                # Huỷ các future còn pending để không block thread pool
                for fut in pending:
                    fut.cancel()
            ...
            for fut in done:
                try:
                    ...
                    logger.warning(f"[{symbol}] Future result error: {e}")

    except Exception as e:
        logger.error(f"[{symbol}] Grid parallel failed entirely: {e}")
...
def run_wfo(
    ...
    Mỗi fold:
      1. Grid search 64 combo trên IS → best params (song song threads)
