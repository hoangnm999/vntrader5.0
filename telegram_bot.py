...
def _fmt_signal(sig) -> str:
    ...
    rs_str = f"RS {sig.rs_value:+.1f}%" if hasattr(sig, "rs_value") else ""
    return "\n".join([
...
async def _do_scan(symbols: list = None) -> list:
    ...
    for sym, df in data.items():
        ...
        sig = compute_signal(sym, df, cfg, df_vni=df_vni, mode="all3")
...
async def cmd_bt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(_fmt_bt(symbol, result), parse_mode="Markdown")
    except Exception as e:
