...
class SymbolResult:
    ...
    def winner(self) -> str:
        """Return which RSI mode has higher PF."""
...
def _walk_forward(
    ...
    df_vni: pd.DataFrame, rsi_mode: str, n_folds: int = 4,
) -> WalkForwardResult:
    ...
    min_pf = float(np.min(pfs))  if pfs else 0.0
    ...
    return WalkForwardResult(
        ...
        avg_pf=round(avg_pf, 3), std_pf=round(std_pf, 3),
        min_pf=round(min_pf, 3), verdict=verdict,
    )
...
def run_symbol(
    ...
    Always includes walk-forward.
    Returns SymbolResult with full comparison data.
    """
    ...
    wf50 = _walk_forward(symbol, df, cfg, bt_cfg, df_vni, "rsi50",
                         bt_cfg["walk_forward_folds"]) if use_walk_forward else None

    # RSI 55 mode
    bt55 = _run_backtest_on_df(symbol, df, cfg, bt_cfg, df_vni, "rsi55")
    wf55 = _walk_forward(symbol, df, cfg, bt_cfg, df_vni, "rsi55",
                         bt_cfg["walk_forward_folds"]) if use_walk_forward else None
...
def format_bt_telegram(symbol: str, result: SymbolResult) -> str:
    """
    Format SymbolResult into a Telegram Markdown message.
    Shows both RSI modes side-by-side + walk-forward for each.
    """
    if result is None:
        return f"❌ Không có dữ liệu backtest cho {symbol}"

    def _v_emoji(v): return {"ROBUST":"✅","MARGINAL":"🟡","WEAK":"❌","THIN_DATA":"⚠️","INCONSISTENT":"🔄"}.get(v,"")
    def _wf_line(wf):
        if not wf: return "  WF: —"
        folds_str = " | ".join(
            f"F{i+1}:{f.profit_factor:.2f}" for i, f in enumerate(wf.folds)
        )
        return f"  WF: {_v_emoji(wf.verdict)}{wf.verdict} avg={wf.avg_pf:.2f}±{wf.std_pf:.2f} min={wf.min_pf:.2f}\n  Folds: {folds_str}"
    ...
    lines = [
        f"📋 *Backtest {symbol}*  —  Winner: `{winner}`",
        "",
        f"*RSI-50 mode* {_v_emoji(b50.verdict)}",
        ...
        _wf_line(w50),
        "",
        f"*RSI-55 mode* {_v_emoji(b55.verdict)}",
