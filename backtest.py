...
def format_bt_telegram(symbol: str, result: SymbolResult) -> str:
    """
    Format SymbolResult into Telegram Markdown message.
    Shows: date range, both RSI modes, walk-forward folds.
    """
    if result is None:
    ...
    def _wf_line(wf: WalkForwardResult) -> str:
        if not wf:
            return "  WF: —"
        if not wf.folds:
        ...
        verdict_line = (
            f"  WF: {_ve(wf.verdict)} {wf.verdict} "
            f"avg={wf.avg_pf:.2f} ±{wf.std_pf:.2f} min={wf.min_pf:.2f}"
        )
    ...
    if result.date_from and result.date_to:
        period = f"\n📅 Kỳ BT: `{result.date_from}` → `{result.date_to}` ({result.total_bars} bars ≈ {result.total_bars//250:.1f} năm)"

    lines = [
        f"📋 *Backtest {symbol}* — Winner: {winner}{period}",
        "",
        f"*RSI\\-50 mode* {_ve(b50.verdict)} `{b50.verdict}`",
        ...
        _wf_line(w50),
        "",
        f"*RSI\\-55 mode* {_ve(b55.verdict)} `{b55.verdict}`",
