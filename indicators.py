# ============================================================
# VN TRADER BOT V5 — indicators.py  (v4 — synced with backtest engine)
#
# FIXES v4:
#   1. RSI calculation đồng bộ với backtest.py _precompute_signals
#      Cũ: close.diff() → delta[0] = NaN → ewm bị lệch
#      Mới: np.diff(prepend=close[0]) → delta[0] = 0 → khớp backtest
#      → scan live và backtest giờ dùng cùng RSI values
#
#   2. VWAP thêm min_periods=1
#      Cũ: .rolling(20).sum() → NaN cho 19 bars đầu
#      Mới: .rolling(20, min_periods=1).sum() → khớp backtest
#      → Không ảnh hưởng scan (đủ 1250 bars) nhưng nhất quán
#
#   3. _ema() helper thêm numpy-based fallback để khớp _ema_np()
#      → Giữ pandas API cho các layer (trend/momentum/volume)
#         nhưng RSI dùng numpy path giống backtest
# ============================================================
...
def _ema_np(arr: np.ndarray, period: int) -> np.ndarray:
    """
    EMA bằng numpy thuần — khớp hoàn toàn với backtest._ema_np().
    Dùng cho RSI để đảm bảo giá trị giống backtest.
    """
    alpha = 2.0 / (period + 1)
...
def _rsi(close: pd.Series, period: int) -> pd.Series:
    """
    FIX v4: Dùng np.diff(prepend=close[0]) thay vì close.diff()
    để khớp với backtest._precompute_signals().

    Cũ: close.diff() → delta[0] = NaN → gain/loss ewm bị lệch từ bar đầu
    Mới: np.diff(prepend) → delta[0] = 0 → gain[0]=loss[0]=0 → RSI khớp backtest
    """
    close_np = close.to_numpy(dtype=float)
    delta    = np.diff(close_np, prepend=close_np[0])  # delta[0] = 0

    gain = _ema_np(np.maximum(delta,  0), period)
    loss = _ema_np(np.maximum(-delta, 0), period)
...
def _vwap_anchored(df: pd.DataFrame, anchor_bars: int = 20) -> pd.Series:
    """
    FIX v4: Thêm min_periods=1 để khớp với backtest._precompute_signals().

    Cũ: .rolling(20).sum() → NaN cho 19 bars đầu
    Mới: .rolling(20, min_periods=1).sum() → không NaN, khớp backtest
    """
    tp  = (df["high"] + df["low"] + df["close"]) / 3
    vol = df["volume"]
    cum_tpv = (tp * vol).rolling(anchor_bars, min_periods=1).sum()
    cum_vol = vol.rolling(anchor_bars, min_periods=1).sum()
    return cum_tpv / cum_vol.replace(0, np.nan)


def _adx_only(high: pd.Series, low: pd.Series, close: pd.Series, period: int) -> pd.Series:
    """Returns only ADX (no DI)."""
    tr = pd.concat([
        high - low,
        (high - close.shift(1)).abs(),
        (low  - close.shift(1)).abs(),
...
def momentum_layer(df: pd.DataFrame, cfg: dict) -> LayerResult:
    ...
    FIX v4: _rsi() giờ dùng numpy path → khớp backtest._precompute_signals()
    """
    close = df["close"]

    rsi_s      = _rsi(close, cfg["rsi_period"])   # FIX: numpy-based RSI
...
def volume_layer(df: pd.DataFrame, cfg: dict) -> LayerResult:
    ...
    Bullish:  Vol/MA20 > vol_multiplier  AND  Price > VWAP
    Bearish:  Vol/MA20 > vol_multiplier  AND  Price < VWAP

    FIX v4: _vwap_anchored() dùng min_periods=1 → khớp backtest
    """
    vol   = df["volume"]
    close = df["close"]
    ...
    vwap      = _vwap_anchored(df)   # FIX: min_periods=1 inside
...
def compute_signal(
    ...
    mode="vol_required" (default)
        ...
        SELL: Volume=-1 + (Trend=-1 OR Momentum=-1)  [RS not required]
